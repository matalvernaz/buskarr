...
def with_home(home, seed, fn):
    """Run fn with $HOME and the bootstrap seed pointed at a scratch tree."""
    old_home, old_seed, old_auth = os.environ.get("HOME"), providers.TIDAL_AUTH_JSON, \
        providers.TIDDL_AUTH
    ...
    try:
        ...
        providers.TIDAL_AUTH_JSON, providers.TIDDL_AUTH = old_seed, old_auth
...
def run():
    ...
    try:
        ...
        ok, why = with_home(home, seed, t.status)
        ...
        print("\nthe outage: a truncated live session, good seed — recoverable")
        write(live, "")
        ...
        ok, _ = with_home(home, seed, t.status)
        ...
        write(live, "")
        ok, why = with_home(home, seed, t.status)
        check("status() rejects a zero-byte auth file with no seed", ok, False)
